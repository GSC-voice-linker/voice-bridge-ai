    ...
    while True:  # 무한 반복
        ...
        if results.multi_hand_landmarks:  # 손이 인식되었는지 확인
            for hand_landmarks in results.multi_hand_landmarks:  # 반복문을 활용해 인식된 손의 주요 부분을 그림으로 그려 표현
                ...
                ## 엄지손가락과 검지 손가락의 각도 구하기

                vector_1 = np.array([hand_landmarks.landmark[4].x, hand_landmarks.landmark[4].y, hand_landmarks.landmark[4].z])
                vector_2 = np.array([hand_landmarks.landmark[8].x, hand_landmarks.landmark[8].y, hand_landmarks.landmark[8].z])
                vector_3 = np.array([hand_landmarks.landmark[0].x, hand_landmarks.landmark[0].y, hand_landmarks.landmark[0].z])
                
                norm_v1 = vector_1 / np.linalg.norm(vector_1)
                norm_v2 = vector_2 / np.linalg.norm(vector_2)
                norm_v3 = vector_3 / np.linalg.norm(vector_3)

                angle_1 = np.arccos(np.dot(norm_v1,norm_v3))
                angle_2 = np.arccos(np.dot(norm_v2,norm_v3))

                angle_d1 = np.degrees(angle_1)
                angle_d2 = np.degrees(angle_2)

                print("angle_d1: ", angle_d1, "angle_d2: ", angle_d2)
                print()
